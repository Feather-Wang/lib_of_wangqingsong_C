...
/*将文件名信息添加到了found链表中，found为一个全局链表*/
       ...
       code in the ZE_ class. */
{
    ...
    //printf("[Debug] newname, [%s][%d] input,name=[%s], isdir=[%d],dosflag=[%d]\n", __FILE__, __LINE__, name, isdir, dosflag);
    /*name进入，原样出来，iname的空间时在ex2in函数中申请的*/
    if ((iname = ex2in(name, isdir, &dosflag)) == NULL)
        return ZE_MEM;
    //printf("[Debug] newname, [%s][%d] output,iname=[%s]\n", __FILE__, __LINE__, iname);

    undosm = NULL;
    /*malloc一块空间，把iname的值放入到里面，地址返回*/
    zname = in2ex(iname);
    /* Convert name to display or OEM name */
    /*检查local_string字符串中是否有ascii码小于40的字符，40在ascii中表示空格*/
    ...
    if ((z = zsearch(zname)) != NULL) {
        ...
        /*获取zip文件的状态信息，只执行一次*/
        if (zipstate == -1){
            zipstate = strcmp(zipfile, "-") != 0 && zstat(zipfile, &zipstatb) == 0;
        }
    ...
    char *d, *s;            /* destination and source file names */
{
    ...
    /*查看压缩文件是否存在，如果存在，判断是否为链接文件，如果不是连接文件，则删除*/
    d_exists = (LSTAT(d, &t) == 0);
    if (d_exists)
    {
        printf("[Debug] replace,[%s][%d]\n", __FILE__, __LINE__);
        /*
    ...
    /*压缩文件在前一步删除了，这里将临时压缩文件改名为压缩文件名*/
    /*如果改名失败，则设置copy=1，采用copy的方式生成压缩文件*/
    if (!copy) {
        printf("[Debug] replace,[%s][%d]\n", __FILE__, __LINE__);
        if (rename(s, d)) {               /* Just move s on top of d */
            ...
            if (errno != EXDEV ) return ZE_CREAT;
        }
    }

    /*前面因为该命压缩文件失败，所以这里尝试采用copy的方式进行拷贝，copy完后会将临时压缩文件删除*/
    if (copy) {
        printf("[Debug] replace,[%s][%d]\n", __FILE__, __LINE__);
...
char *local_to_display_string(char *local_string)
{
    ...
    temp_string = (char *)malloc(2 * strlen(local_string) + 1);
    ...
    *cp_dst = '\0';
    ...
    display_string = (char *)malloc(strlen(temp_string) + 1);
