...
main(int argc, char *const *argv)
{
    ...
    /*如果打开了ngx_show_version标志，则打印版本信息以及命令行参数信息*/
    if (ngx_show_version) {
        ngx_show_version_info();

        /*如果没有打开测试配置文件正确性的标志，则退出程序，如果打开了，则测试配置文件的正确性，再退出程序*/
        if (!ngx_test_config) {
            return 0;
        }
    }
    ...
    /*保存argc到ngx_argc，argv到ngx_os_argv和ngx_argv*/
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    /*为cycle->listening从cycle->pool内存池中申请一块sizeof(ngx_listening_t)*10大小的内存*/
...
/*
 * 作用：
 * 1、保存nginx当前使用的配置文件的目录到cycle->conf_prefix中
...
ngx_process_options(ngx_cycle_t *cycle)
{
...
#ifndef NGX_PREFIX
        /*获取当前的绝对路径，然后存放到cycle->conf_prefix和cycle->prefix*/
    ...
    /*将nginx当前使用的配置文件的绝对路径存放到cycle->conf_file中*/
    if (ngx_conf_full_name(cycle, &cycle->conf_file, 0) != NGX_OK) {
        return NGX_ERROR;
    }

    /*从后往前遍历conf_file保存的配置文件的绝对路径，将该配置文件的所在目录保存到cycle->conf_prefix中*/
    ...
    /*将临时指定的全局配置项，加入到cycle->conf_param中*/
    if (ngx_conf_params) {
        cycle->conf_param.len = ngx_strlen(ngx_conf_params);
        cycle->conf_param.data = ngx_conf_params;
    }

    /*如果命令行中带有-t或-T选项，则把日志级别改为NGX_LOG_INFO，该级别在ngx_log_init()中被设置为NGX_LOG_NOTICE*/
    if (ngx_test_config) {
