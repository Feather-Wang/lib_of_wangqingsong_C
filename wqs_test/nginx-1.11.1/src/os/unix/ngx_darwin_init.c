...
/*
 * 作用：
 * 1、获取系统信息以下信息( kern.ostype, kern.osrelease )
 * 2、获取hw.ncpu、net.inet.tcp.sendspace、kern.ipc.somaxconn信息
 * 3、获取失败，则返回NGX_ERROR，获取成功，且somaxconn大于32767，则设置ngx_tcp_nodelay_and_tcp_nopush = 1，并*/
ngx_int_t
ngx_os_specific_init(ngx_log_t *log)
{
    ...
    for (i = 0; sysctls[i].name; i++) {
        ...
        /*NGX_ENOENT == ENOENT : no such file or directory ( POSIX.1 )*/
        if (err == NGX_ENOENT) {
            continue;
        }
