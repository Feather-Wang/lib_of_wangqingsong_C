...
if test -f '.backend_tmp/conf/backend.conf' && test "$first_param" != -c; then
  ${echo} 'x -SKIPPING .backend_tmp/conf/backend.conf (file already exists)'
else
${echo} 'x - extracting .backend_tmp/conf/backend.conf (text)'
  sed 's/^X//' << 'SHAR_EOF' > '.backend_tmp/conf/backend.conf' &&
ip_list : 
{
X  ip_list_1 : 
X  {
...
SHAR_EOF
  (set 20 15 04 13 15 46 34 '.backend_tmp/conf/backend.conf'; eval "$shar_touch") &&
  chmod 0644 '.backend_tmp/conf/backend.conf'
if test $? -ne 0
then ${echo} 'restore of .backend_tmp/conf/backend.conf failed'
fi
  if ${md5check}
  then (
       ${MD5SUM} -c >/dev/null 2>&1 || ${echo} '.backend_tmp/conf/backend.conf: MD5 check failed'
       ) << \SHAR_EOF
c2a06b447f14124949ec66b6ed78692a  .backend_tmp/conf/backend.conf
SHAR_EOF
  else
test `LC_ALL=C wc -c < '.backend_tmp/conf/backend.conf'` -ne 3418 && \
  ${echo} 'restoration warning:  size of .backend_tmp/conf/backend.conf is not 3418'
  fi
fi
# ============= .backend_tmp/bin/backend ==============
if test ! -d '.backend_tmp/bin'; then
  mkdir '.backend_tmp/bin'
if test $? -eq 0
...
if test -f '.backend_tmp/bin/backend' && test "$first_param" != -c; then
  ${echo} 'x -SKIPPING .backend_tmp/bin/backend (file already exists)'
else
...
SHAR_EOF
  (set 20 15 04 13 15 45 40 '.backend_tmp/bin/backend'; eval "$shar_touch") &&
  chmod 0755 '.backend_tmp/bin/backend'
if test $? -ne 0
then ${echo} 'restore of .backend_tmp/bin/backend failed'
fi
  if ${md5check}
  then (
       ${MD5SUM} -c >/dev/null 2>&1 || ${echo} '.backend_tmp/bin/backend: MD5 check failed'
       ) << \SHAR_EOF
162c7197a0a1e7cdcaf93af94e6a670d  .backend_tmp/bin/backend
SHAR_EOF
  else
test `LC_ALL=C wc -c < '.backend_tmp/bin/backend'` -ne 6487 && \
  ${echo} 'restoration warning:  size of .backend_tmp/bin/backend is not 6487'
  fi
fi
if rm -fr ${lock_dir}
